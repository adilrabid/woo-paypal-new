...
class PayPal_Request_API_Injector {
        ...
        public function create_paypal_subscription_for_billing_plan( $plan_id, $data = array(), $additional_args = array()){
            ...
            if ( $response !== false){
                ...
                PayPal_Utils::log('Create-subscription response. Subscription ID: '. $created_sub_id, true);
        ...
        */
        public function create_paypal_order_by_url_and_args( $data, $additional_args = array(), $pu_items = array() ){
            ...
            if( is_array($pu_items) && !empty($pu_items)) {
                ...
                // Grand total must equal = (item_total + tax_total + shipping + handling + insurance) - shipping_discount - discount.
            ...
            //Debugging purpose.
            //PayPal_Utils::log('Order-create request data below.', true);
            //PayPal_Utils::log_array( $order_data, true );

            //Get the environment mode.
            $environment_mode = PayPal_Utils::get_api_environment_mode_from_settings();
            ...
            PayPal_Utils::log('Executing order create (v2/checkout/orders) using URL and args.', true);
            ...
            if ( $response !== false){
                ...
                PayPal_Utils::log('Order-create response. Order ID: '. $created_order_id, true);
        ...
        public function capture_paypal_order( $order_id, $additional_args = array() ){
            if( empty($order_id) ){
                PayPal_Utils::log('Empty PayPal order ID received. cannot process this request.', false);
                return false;
            }
