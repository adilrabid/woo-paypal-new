...
class PayPal_JS_Button_Embed {
	...
	*/
    public $button_id_prefix = 'wcpprog_paypal_button_';
	...
	public function generate_paypal_js_sdk_args( $args = array()){
		...
		if( empty($sdk_args['client-id']) ){
			...
			PayPal_Utils::log( 'PayPal client ID is missing in the settings for environment mode: ' . $env_mode_name, false );
		}
		 ...
		 * * For example 'generate_paypal_js_sdk_args' is actually 'wp_estore_generate_paypal_js_sdk_args' for simple cart plugin.
		 */ 
		$sdk_args = apply_filters( PayPal_Utils::hook('generate_paypal_js_sdk_args'), $sdk_args );
		return $sdk_args;
	}
	...
	public function load_paypal_sdk() {
		$args = $this->settings_args;//The args are set before this function is called from the button's code.
		$sdk_args = $this->generate_paypal_js_sdk_args($args);

		$script_url = add_query_arg( $sdk_args, 'https://www.paypal.com/sdk/js' );
		?>
		<script type="text/javascript">
			wcpprog_onDocumentReady(function(){
			...
			function wcpprog_onDocumentReady(callback) {
	...
	public function load_paypal_sdk_for_subscriptions() {
		$args = $this->settings_args_subscription; //The sub-args are set before this function is called from the button's code.
		$sdk_args = $this->generate_paypal_js_sdk_args($args);
		...
		<script type="text/javascript">
			...
			* REPLACE: plugin prefix across different plugins.
			*/
			script_sub.setAttribute( 'data-namespace', 'paypal_subscriptions' );//Use a different namespace for the subscription buttons.
			script_sub.async = true;
			script_sub.src = '<?php echo esc_url_raw( $script_url ); ?>';
			script_sub.onload = function() {
				document.dispatchEvent(new Event('wcpprog_paypal_sdk_subscriptions_loaded'));//REPLACE: plugin prefix across different plugins.
			};
