...
class PayPal_Bearer {
	...
	const BEARER_CACHE_KEY = 'ppcp-bearer-cache-key';
	...
	public function get_bearer_token( $environment_mode = 'production' ) {
		...
		if ( $token_exists ) {
			...
			//PayPal_Utils::log('Cached bearer token exists. Checking if it is valid.', true);
			...
			if ( $is_valid_token ) {
				...
				PayPal_Utils::log('Using the cached PayPal API access token (since it is still valid). Environment mode: ' . $environment_mode, true);
				return $token_string;
			}
		}
	...
	public function create_new_bearer_token( $environment_mode = '' ) {
		...
		if( empty( $client_id ) || empty( $secret ) ){
			PayPal_Utils::log('PayPal API credentials are not set. Missing Client ID or Secret Key. Please set them in the plugin\'s payment settings page.', false);
			return false;
		}
		...
		PayPal_Utils::log('PayPal access token created successfully.', true);
