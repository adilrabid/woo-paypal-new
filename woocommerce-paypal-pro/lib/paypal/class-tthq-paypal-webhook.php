...
class PayPal_Webhook {
	...
	public function __construct() {
		...
		$this->id = PayPal_Utils::get_option( 'paypal_webhook_id_' . $this->mode );
	}
	...
	public function set_id( $id ) {
		PayPal_Utils::update_option( 'paypal_webhook_id_' . $this->mode, $id );
	}
	...
	public function check_webhook() {
		...
		if( is_wp_error( $response ) ){
			...
			if ( $error_code === 'INVALID_RESOURCE_ID' ) {
				$ret['msg'] = __( 'No webhook found. Use the following Create Webhook button to create a new webhook automatically in your PayPal account.', 'woocommerce-paypal-pro-payment-gateway');
		...
		$ret['msg']     = __( 'Webhook exists. If you still have issues with webhooks, you can delete it and create again.', 'woocommerce-paypal-pro-payment-gateway' );
		$ret['hidebtn'] = true;
		return $ret;
	}
	...
	public function check_and_create_webhook(){
		...
		if ( $protocol !== 'https' ) {
			$ret['status'] = 'no';
			$ret['msg']    = __( 'Invalid webhook URL.', 'woocommerce-paypal-pro-payment-gateway' )  . ': ' . __( 'Note that the PayPal subscription API requires your site to use HTTPS URLs. You must use an SSL certificate with HTTPS URLs to complete the setup of the subscription addon and use it.', 'woocommerce-paypal-pro-payment-gateway' );
			return $ret;
		}
		...
		if ( ! is_wp_error( $response ) ) {
			...
			$ret['status']  = 'yes';
			$ret['hidebtn'] = true;
			$ret['msg'] = __( 'Webhook has been created.', 'woocommerce-paypal-pro-payment-gateway' );
		} else {
	...
	public function check_and_delete_webhook() {
		...
		if ( $ret['status'] == 'yes' ) {
			...
			if ( !is_wp_error( $response )){
				//Webhook deleted.
				$ret['success'] = true;
				$ret['msg'] = __( 'Webhook has been deleted.', 'woocommerce-paypal-pro-payment-gateway' );
				return $ret;
			} else {
		...
		$ret = array();
		$ret['success'] = false;
		$ret['msg'] = __( 'No webhook found. Nothing to delete.', 'woocommerce-paypal-pro-payment-gateway' );
		//No webhook exists in PayPal. Delete the webhook ID (if any) from our DB to clean it up (so it can be created again).
		PayPal_Utils::delete_option( 'paypal_webhook_id_' . $this->mode );
	...
	public function check_and_create_webhook_for_live_mode(){
		if( !empty($this->live_client_id) && !empty($this->live_secret) ){
			...
			PayPal_Utils::log( 'Live mode API credentials are not set.', true );
			$ret = array();
			$ret['success'] = false;
			$ret['msg'] = __( 'Live mode credentials are not set. Cannot create webhook.', 'woocommerce-paypal-pro-payment-gateway' );
			return $ret;
		}
	}
	...
	public function check_and_create_webhook_for_sandbox_mode(){
		if( !empty($this->sandbox_client_id) && !empty($this->sandbox_secret) ){
			...
			PayPal_Utils::log( 'Sandbox mode API credentials are not set.', true );
			$ret = array();
			$ret['success'] = false;
			$ret['msg'] = __( 'Sandbox mode credentials are not set. Cannot create webhook.', 'woocommerce-paypal-pro-payment-gateway' );
			return $ret;			
		}
	}
	...
	public function check_and_create_webhooks_for_both_modes() {
		...
		if( isset( $ret['status']) && $ret['status'] == 'no' ){
			...
			PayPal_Utils::log( 'Webhook creation failed for live mode. Error: ' . $ret['msg'], true );
		}
		...
		if( isset( $ret['status']) && $ret['status'] == 'no' ){
			//Webhook creation failed. 
			PayPal_Utils::log( 'Webhook creation failed for sandbox mode. Error: ' . $ret['msg'], true );
		}
