...
use TTHQ\WC_PP_PRO\Lib\PayPal\PayPal_Request_API;
use TTHQ\WC_PP_PRO\Lib\PayPal\PayPal_Bearer;
...
class PayPal_PPCP_Onboarding_Serverside {
	...
	public function __construct() {
		...
		add_action(PayPal_Utils::hook('handle_onboarded_callback_data', true), array($this, 'handle_onboarded_callback_data'));
		add_action(PayPal_Utils::hook('handle_onboarded_callback_data', true, true), array($this, 'handle_onboarded_callback_data'));

		add_action(PayPal_Utils::hook('ppcp_delete_cache', true), array($this, 'handle_ppcp_delete_cache'));

		add_action(PayPal_Utils::hook('ppcp_disconnect_production', true), array($this, 'handle_ppcp_disconnect_production'));
		add_action(PayPal_Utils::hook('ppcp_disconnect_sandbox', true), array($this, 'handle_ppcp_disconnect_sandbox'));
	...
	public function handle_onboarded_callback_data() {
		...
		PayPal_Utils::log('Onboarding step: handle_onboarded_callback_data.', true);
		...
		if (empty($data)) {
			wp_send_json(
				array(
					'success' => false,
					'msg'  => __('Empty data received.', 'woocommerce-paypal-pro-payment-gateway'),
				)
			);
		}
		...
		// PayPal_Utils::log_array( $data_array, true );
		...
		if (! check_ajax_referer($nonce_string, '_wpnonce', false)) {
			wp_send_json(
				array(
					'success' => false,
					'msg'  => __('Nonce check failed. The page was most likely cached. Please reload the page and try again.', 'woocommerce-paypal-pro-payment-gateway'),
				)
			);
			exit;
		}
		...
		if (! $access_token) {
			...
			wp_send_json(
				array(
					'success' => false,
					'msg'  => __('Failed to generate access token. check debug log file for any error message.', 'woocommerce-paypal-pro-payment-gateway'),
				)
			);
			exit;
		}
		...
		//PayPal_Utils::log( 'Onboarding step: access token generated successfully. Token: ' . $access_token, true );//Debug purpose only
		...
		if (! $seller_api_credentials) {
			...
			wp_send_json(
				array(
					'success' => false,
					'msg'  => __('Failed to get seller API credentials. check debug log file for any error message.', 'woocommerce-paypal-pro-payment-gateway'),
				)
			);
		}
		...
		//PayPal_Utils::log( 'Onboarding step: using access token from the previous step. Token: ' . $bearer_token, true );//Debug purpose only
		...
		PayPal_Utils::log_array($seller_account_status, true);
		if (! $seller_account_status) {
			//Failed to get seller account status.
			wp_send_json(
				array(
					'success' => false,
					'msg'  => __('Failed to get seller account status. check debug log file for any error message.', 'woocommerce-paypal-pro-payment-gateway'),
				)
			);
		}
		...
		if (! $seller_account_status['payments_receivable']) {
			...
			wp_send_json(
				array(
					'success' => false,
					'msg'  => __('Your PayPal account is limited so you cannot accept payment. Contact PaPal support or check your PayPal account inbox for an email from PayPal for the next steps to remove the account limit.', 'woocommerce-paypal-pro-payment-gateway'),
				)
			);
		}
		if (! $seller_account_status['primary_email_confirmed']) {
			...
			wp_send_json(
				array(
					'success' => false,
					'msg'  => __('Your PayPal account email is not confirmed. Check your PayPal account inbox for an email from PayPal to confirm your PayPal email address.', 'woocommerce-paypal-pro-payment-gateway'),
				)
			);
		}
		PayPal_Utils::log('PPCP_STANDARD vetting_status: ' . $seller_account_status['ppcp_standard_vetting_status'], true);
		PayPal_Utils::log('CUSTOM_CARD_PROCESSING status: ' . $seller_account_status['custom_card_processing_status'], true);
		...
		PayPal_Utils::log('Successfully processed the handle_onboarded_callback_data. Environment mode: ' . $environment_mode, true);

		//If everything is processed successfully, send the success response.
		wp_send_json(
			array(
				'success' => true,
				'msg' => 'Successfully processed the handle_onboarded_callback_data.'
			)
		);
	}
	...
	public function get_seller_account_status_data_using_bearer_token($bearer_token, $seller_api_credentials, $environment_mode = 'production') {
		PayPal_Utils::log('Onboarding step: get_seller_account_status_data. Environment mode: ' . $environment_mode, true);

		$api_base_url = PayPal_Utils::get_api_base_url_by_environment_mode($environment_mode);
		$partner_id = PayPal_Utils::get_partner_id_by_environment_mode($environment_mode);
		...
		//Debug purpose only
		//PayPal_Utils::log( 'PayPal API request headers for getting seller account status: ', true );
		//PayPal_Utils::log_array( $args, true);		
		...
		if (is_wp_error($response)) {
			...
			PayPal_Utils::log('Failed to post the request to the PayPal API. Error: ' . $error_msg, false);
			return false;
		}
		...
		if ($status_code !== 200) {
			...
			PayPal_Utils::log('PayPal API returned an error. Status Code: ' . $status_code . ' Response Body: ' . $response_body, false);
			return false;
		}
		...
		if (! isset($json->payments_receivable) || ! isset($json->primary_email_confirmed)) {
			...
			PayPal_Utils::log('Failed to get seller PayPal account status. Status code: ' . $status_code . ', Error msg: ' . $error_msg, false);
			return false;
		}
	...
	public function save_seller_paypal_email($seller_paypal_email, $environment_mode = 'production') {
		...
		if ($environment_mode == 'sandbox') {
			$settings->set_value(PayPal_PPCP_Config::key('sandbox_seller_paypal_email'), $seller_paypal_email);
		} else {
			$settings->set_value(PayPal_PPCP_Config::key('live_seller_paypal_email'), $seller_paypal_email);
		}

		$settings->save();
		PayPal_Utils::log('Seller PayPal email address (' . $seller_paypal_email . ') saved successfully (environment mode: ' . $environment_mode . ').', true);
	}
	...
	public function save_seller_api_credentials($seller_api_credentials, $environment_mode = 'production') {
		...
		if ($environment_mode == 'sandbox') {
			$settings->set_value(PayPal_PPCP_Config::key('sandbox_client_id'), $seller_api_credentials['client_id']);
			$settings->set_value(PayPal_PPCP_Config::key('sandbox_client_secret'), $seller_api_credentials['client_secret']);
			$settings->set_value(PayPal_PPCP_Config::key('sandbox_seller_merchant_id'), $seller_api_credentials['payer_id']);
		} else {
			$settings->set_value(PayPal_PPCP_Config::key('live_client_id'), $seller_api_credentials['client_id']);
			$settings->set_value(PayPal_PPCP_Config::key('live_client_secret'), $seller_api_credentials['client_secret']);
			$settings->set_value(PayPal_PPCP_Config::key('live_seller_merchant_id'), $seller_api_credentials['payer_id']);
		}

		$settings->save();
		PayPal_Utils::log('Seller API credentials (environment mode: ' . $environment_mode . ') saved successfully.', true);
	}
	...
	public static function reset_seller_api_credentials($environment_mode = 'production') {
		...
		if ($environment_mode == 'sandbox') {
			$settings->set_value(PayPal_PPCP_Config::key('sandbox_client_id'), '');
			$settings->set_value(PayPal_PPCP_Config::key('sandbox_client_secret'), '');
			$settings->set_value(PayPal_PPCP_Config::key('sandbox_seller_merchant_id'), '');
			$settings->set_value(PayPal_PPCP_Config::key('sandbox_seller_paypal_email'), '');
		} else {
			$settings->set_value(PayPal_PPCP_Config::key('live_client_id'), '');
			$settings->set_value(PayPal_PPCP_Config::key('live_client_secret'), '');
			$settings->set_value(PayPal_PPCP_Config::key('live_seller_merchant_id'), '');
			$settings->set_value(PayPal_PPCP_Config::key('live_seller_paypal_email'), '');
		}
		...
		//Save the settings
		$settings->save();
		PayPal_Utils::log('Seller API credentials (environment mode: ' . $environment_mode . ') reset/removed successfully.', true);

		//Delete any cached token using the old credentials (so it is forced to generate and cache a new one after onboarding (when new API call is made)))
		PayPal_Bearer::delete_cached_token();
		PayPal_Utils::log('Executed delete PayPal bearer cached token function (to clean it up).', true);
	}
	...
	public function generate_token_using_shared_id($shared_id, $auth_code, $environment_mode = 'production') {
		PayPal_Utils::log('Onboarding step: generate_token_using_shared_id. Environment mode: ' . $environment_mode, true);

		if (isset($environment_mode) && $environment_mode == 'sandbox') {
			$query_args = PayPal_Utils::get_option('ppcp_connect_query_args_' . $environment_mode);
			$seller_nonce = isset($query_args['sellerNonce']) ? $query_args['sellerNonce'] : '';
		} else {
			$query_args = PayPal_Utils::get_option('ppcp_connect_query_args_' . $environment_mode);
			$seller_nonce = isset($query_args['sellerNonce']) ? $query_args['sellerNonce'] : '';
		}
		PayPal_Utils::log('Seller nonce value: ' . $seller_nonce, true);
		...
		//PayPal_Utils::log_array( $args, true);//Debugging purpose
		$response = PayPal_Request_API::send_request_by_url_and_args($url, $args);
		//PayPal_Utils::log_array( $response, true);//Debugging purpose
		...
		if (! isset($json->access_token)) {
			...
			PayPal_Utils::log('Failed to generate token. Status code: ' . $status_code . ', Error msg: ' . $error_msg, false);
			return false;
		}
	...
	public function get_seller_api_credentials_using_token($access_token, $environment_mode = 'production') {
		PayPal_Utils::log('Onboarding step: get_seller_api_credentials_using_token. Environment mode: ' . $environment_mode, true);

		$api_base_url = PayPal_Utils::get_api_base_url_by_environment_mode($environment_mode);
		$partner_merchant_id = PayPal_Utils::get_partner_id_by_environment_mode($environment_mode);
		...
		if (! isset($json->client_id) || ! isset($json->client_secret)) {
			...
			PayPal_Utils::log('Failed to get seller API credentials. Status code: ' . $status_code . ', Error msg: ' . $error_msg, false);
			return false;
		}
	...
	public function handle_ppcp_delete_cache(){
		if (! check_ajax_referer(PayPal_Utils::auto_prefix('ppcp_delete_cache_nonce'), '_wpnonce', false)) {
			wp_send_json(
				array(
					'success' => false,
					'msg'  => __('Nonce check failed. The page was most likely cached. Please reload the page and try again.', 'woocommerce-paypal-pro-payment-gateway'),
		...
		wp_send_json(
			array(
				'success' => true,
				'msg'  => __('PayPal API access token cache deleted successfully.', 'woocommerce-paypal-pro-payment-gateway'),
			)
		);
	}

	public function handle_ppcp_disconnect_production(){
		if (! check_ajax_referer(PayPal_Utils::auto_prefix('ac_disconnect_nonce_production'), '_wpnonce', false)) {
			wp_send_json(
				array(
					'success' => false,
					'msg'  => __('Nonce check failed. The page was most likely cached. Please reload the page and try again.', 'woocommerce-paypal-pro-payment-gateway'),
		...
		self::reset_seller_api_credentials('production');

		wp_send_json(
			array(
				'success' => true,
				'msg'  => __('PayPal account disconnected.', 'woocommerce-paypal-pro-payment-gateway'),
			)
		);
	}	
	
	public function handle_ppcp_disconnect_sandbox(){
		if (! check_ajax_referer(PayPal_Utils::auto_prefix('ac_disconnect_nonce_sandbox'), '_wpnonce', false)) {
			wp_send_json(
				array(
					'success' => false,
					'msg'  => __('Nonce check failed. The page was most likely cached. Please reload the page and try again.', 'woocommerce-paypal-pro-payment-gateway'),
		...
		self::reset_seller_api_credentials('sandbox');

		wp_send_json(
			array(
				'success' => true,
				'msg'  => __('PayPal sandbox account disconnected.', 'woocommerce-paypal-pro-payment-gateway'),
			)
		);
	}
}
