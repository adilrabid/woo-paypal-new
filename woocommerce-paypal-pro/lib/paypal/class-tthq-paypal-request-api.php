...
class PayPal_Request_API {
	...
	public static function get_paypal_auth_assertion_value($environment_mode){
		$partner_client_id = PayPal_Utils::get_partner_client_id_by_environment_mode( $environment_mode );
		$seller_merchant_id = PayPal_Utils::get_seller_merchant_id_by_environment_mode( $environment_mode );
	...
	public function get( $endpoint, $params = array(), $additional_args = array() ) {
		...
		$args['timeout'] = apply_filters( PayPal_Utils::hook('paypal_api_request_timeout'), 60 );
	...
	private function process_request_result( $res, $status_code = 200, $additional_args = array() ) {
		...
		//=== Debug purposes (Useful for PayPal Tech Support) ===
		// PayPal_Utils::log( '----- PayPal REST API response output -----', true );
		// $paypal_debug_id_and_info = PayPal_Request_API::get_paypal_debug_id_and_info($res, $additional_args);
		// PayPal_Utils::log( $paypal_debug_id_and_info, true );
		// PayPal_Utils::log_array( $response_body_json_decoded, true );
		//=== End of debug purposes ===
	...
	public function log_api_error_response( $response, $error_message, $additional_args = array() ) {
		...
		PayPal_Utils::log( 'Error occurred with the PayPal API request. Error message: ' . $error_message, false );
		...
		PayPal_Utils::log( $paypal_debug_id_and_info, false );
	}
	...
	public static function send_request_by_url_and_args( $url, $args ) {
		 ...
		 */ 
		$args = apply_filters( PayPal_Utils::hook('paypal_api_request_by_url_args'), $args, $url );
		if ( ! isset( $args['headers']['PayPal-Partner-Attribution-Id'] ) ) {
			$args['headers']['PayPal-Partner-Attribution-Id'] = 'TipsandTricks_SP_PPCP';
		}

		//=== Debug purposes (Useful for PayPal Tech Support) ===
		// PayPal_Utils::log( '----- PayPal API request header -----', true );
		// PayPal_Utils::log_array( $args, true );
		//=== End of debug purposes ===
		...
		if ( is_wp_error($response) ) {
			...
			PayPal_Utils::log( 'Error occurred with the PayPal API (by URL) request. Error message: ' . $error_message, false );
			// Log the PayPal debug id for PayPal API Error.
			$paypal_debug_id = wp_remote_retrieve_header( $response, 'paypal-debug-id' );
			PayPal_Utils::log( 'PayPal Debug ID: ' . $paypal_debug_id . ', Request URL: ' . $url , false );			
			return false;
		}

		//PayPal Debug ID
		$paypal_debug_id = wp_remote_retrieve_header( $response, 'paypal-debug-id' );
		PayPal_Utils::log( 'PayPal Debug ID from the REST API (by URL) request. Debug ID: ' . $paypal_debug_id . ', Request URL: ' . $url, true );
		...
		//PayPal_Utils::log( 'PayPal API (by URL) response body: ' . $debug_api_response, true );
		//=== End of debug purposes ===
