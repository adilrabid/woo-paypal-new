...
class PayPal_ACDC_Related {
	...
	public function setup_acdc_related_ajax_request_actions() {
		...
		add_action( PayPal_Utils::hook('acdc_setup_order', true), array(&$this, 'acdc_setup_order' ) );
		add_action( PayPal_Utils::hook('acdc_setup_order', true, true), array(&$this, 'acdc_setup_order' ) );

		//Handle the ajax request for ACDC 'Buy Now' type buttons capture order.
		add_action( PayPal_Utils::hook('acdc_capture_order', true), array(&$this, 'acdc_capture_order' ) );
		add_action( PayPal_Utils::hook('acdc_capture_order', true, true), array(&$this, 'acdc_capture_order' ) );
	}
	...
	public static function get_sdk_src_url_for_acdc( $environment_mode = 'production', $currency = 'USD' ){
		...
		$client_id = PayPal_Utils::get_seller_client_id_by_environment_mode( $environment_mode );
        $merchant_id = PayPal_Utils::get_seller_merchant_id_by_environment_mode( $environment_mode );
		...
		if(!empty($merchant_id)){
			...
			$pp_acdc_msg_str = __( 'Note: Merchant ID value is empty so the SDK URL will not include this parameter.', 'woocommerce-paypal-pro-payment-gateway' );
			PayPal_Utils::log($pp_acdc_msg_str, true);
		}
	...
	public function acdc_setup_order(){
		PayPal_Utils::log( 'Received request - acdc_setup_order', true);
		...
		PayPal_Utils::log( 'acdc_setup_order ajax request received for createOrder. Button ID: '.$button_id.', On Page Button ID: ' . $on_page_button_id, true );
		...
		// PayPal_Utils::log('--- Var Export Below ---', true);
		// $debug = var_export($response, true);
		// PayPal_Utils::log($debug, true);
		...
		if($response !== false){
			...
			wp_send_json(
				array(
					'success' => false,
					'err_msg'  => __( 'Failed to create the order. Enable the debug logging feature to get more details.', 'woocommerce-paypal-pro-payment-gateway' ),
				)
			);
			exit;
		}

        PayPal_Utils::log( 'acdc_setup_order done. PayPal Order ID: ' . $paypal_order_id, true );
	...
	public function acdc_capture_order(){
		...
		//$button_id = isset( $data['button_id'] ) ? sanitize_text_field( $data['button_id'] ) : '';
		PayPal_Utils::log( 'Received request - acdc_capture_order. Order ID: ' . $order_id . ', on_page_button_id: ' . $on_page_button_id, true );
